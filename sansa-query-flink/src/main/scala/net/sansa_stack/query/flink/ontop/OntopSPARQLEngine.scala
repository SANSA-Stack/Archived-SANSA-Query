...
class OntopSPARQL2SQLRewriter(val partitions: Set[RdfPartitionComplex],
  ...
  ontopProperties.load(getClass.getClassLoader.getResourceAsStream("ontop-flink.properties"))
...
//      }
    ...
    result = result.select(signature.asScala.map(v => v.getName).map(name => $"$name"): _*)
  ...
  def executeDebug(query: String): (Table, Option[OntopQueryRewrite]) = {
    ...
    try {
      ...
      case e: EmptyQueryException =>
        logger.warn(s"Empty SQL query generated by Ontop. Returning empty DataSet for SPARQL query\n$query")
        val emptyTable = env.fromValues(Iterable.empty[Row])
        (emptyTable, None)
      case e: Exception =>
        logger.error(s"Flink failed to execute translated SQL query\n$query", e)
        throw e
...
object OntopSPARQLEngine {
  ...
  def apply(env: BatchTableEnvironment, databaseName: String, partitions: Set[RdfPartitionComplex], ontology: Option[OWLOntology]): OntopSPARQLEngine
